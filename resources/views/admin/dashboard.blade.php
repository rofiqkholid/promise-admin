<x-app-layout>
    ...
    <div class="px-6 py-4 flex flex-col gap-4 h-auto lg:h-[calc(100vh-120px)] lg:max-h-[calc(100vh-120px)] overflow-y-auto lg:overflow-hidden" x-data="dashboardConsole()">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 shrink-0">
            <!-- Total Users -->
            <div class="bg-white border border-gray-200 p-3.5 flex items-center gap-3 shadow-[0_1px_2px_rgba(0,0,0,0.02)] rounded-sm">
            ...
            <div class="lg:col-span-8 flex flex-col gap-4 min-h-0">
                ...
                <div class="bg-white border border-gray-200 flex-1 min-h-0 flex flex-col rounded-sm shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                    ...
                    <div class="flex-1 min-h-0 py-2.5 px-4 flex flex-col sm:flex-row items-center gap-6 justify-center">
                        ...
                        <!-- Custom Legend -->
                        <div class="flex-1 space-y-2 max-w-xs w-full">
                            <template x-for="(scope, index) in scopeChartData" :key="scope.id">
                                <div class="flex items-center justify-between text-[11px] border-b border-slate-100 pb-1.5 last:border-0 last:pb-0">
                                    <div class="flex items-center gap-2">
                                        <div class="w-2.5 h-2.5 rounded-full shrink-0" :style="'background-color: ' + getScopeColor(scope.id)"></div>
                                        <span class="font-semibold text-slate-600 truncate max-w-44" x-text="scope.label"></span>
                                    </div>
                                    <span class="font-bold font-mono text-slate-800" x-text="scope.count + ' Users'"></span>
                                </div>
                            </template>
                        </div>
                    </div>
                            ...
                            <template x-for="role in roleBreakdown">
                                <div>
                                    <div class="flex justify-between items-center text-[10px] font-semibold text-slate-600 mb-1">
                                        ...
                                        <span class="font-mono font-bold text-slate-800" x-text="role.user_count + ' Users'"></span>
                                    </div>
                                    <div class="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div class="h-full bg-gradient-to-r from-violet-500 to-indigo-600 transition-all duration-500" :style="'width: ' + ((role.user_count / stats.total_users) * 100) + '%'"></div>
                                    </div>
                                </div>
                            </template>
                        </div>
                    ...
                    <div class="px-4 border-b border-gray-200 bg-gray-50 flex items-center shrink-0">
                        <div class="flex items-center gap-4">
                            ...
                            <button @click="rightTab = 'online'" 
                                ...
                                <span class="text-[9px] font-mono bg-emerald-50 text-emerald-600 px-1 rounded-xs font-bold" x-text="stats.online_users"></span>
                            </button>
                        </div>
                    </div>
                                        ...
                                        <tr class="hover:bg-slate-50/70 transition-colors">
                                            <td class="py-3 px-4 font-medium text-slate-700">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <div class="min-w-0">
                                                        <div class="truncate w-32 font-bold text-slate-800 text-[11px] leading-tight" title="{{ $user->name }}">{{ $user->name }}</div>
                                            ...
                                            <td class="py-3 px-3 font-mono text-slate-500 text-[10px]">{{ $user->nik }}</td>
                                            <td class="py-3 px-4 text-right font-mono text-slate-400 text-[10px]">{{ $user->created_at ? $user->created_at->format('m-d H:i') : '—' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                                        ...
                                        <tr class="hover:bg-slate-50/70 transition-colors">
                                            <td class="py-3 px-4 font-medium text-slate-700">
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <div class="min-w-0">
                                                        <div class="truncate w-32 font-bold text-slate-800 text-[11px] leading-tight" :title="user.name" x-text="user.name"></div>
                                            ...
                                            <td class="py-3 px-3 font-mono text-slate-500 text-[10px]" x-text="user.nik"></td>
                                            <td class="py-3 px-4 text-right">
                                                <span class="inline-block px-1.5 py-0.5 rounded-xs bg-emerald-50 text-emerald-600 font-bold text-[9px] border border-emerald-100" x-text="formatLastActive(user.last_activity)"></span>
                                            </td>
                                        </tr>
                                    </template>
                                </tbody>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <template x-for="user in overrideUsers" :key="user.nik">
                                    <tr class="hover:bg-slate-50/70 transition-colors">
                                        ...
                                        <td class="py-3 px-3 font-mono text-slate-500 text-[10px]" x-text="user.nik"></td>
                                        <td class="py-3 px-4 text-right">
                                            <span class="inline-block px-1.5 py-0.5 rounded-xs bg-rose-50 text-rose-600 font-bold font-mono text-[9px] border border-rose-100" x-text="user.override_count + ' Rules'"></span>
                                        </td>
                                    </tr>
                                </template>
                            </tbody>
